...
export function OfflineExitSync({ onComplete, onCancel }: OfflineExitSyncProps) {
  ...
  const handleDownloadSummary = useCallback(() => {
    ...
    const lines = isId
      ...
      : [
          "===================================",
          "  SPRINGHUB — Survey Summary",
          "===================================",
          "",
          `Start Time: ${summary.startTime ? new Date(summary.startTime).toLocaleString("en-US") : "—"}`,
          `Total Distance: ${formatDistance(summary.totalDistance)}`,
          "",
          "--- Markers ---",
          `💧 Springs: ${summary.springCount}`,
          `🌳 Trees: ${summary.treeCount}`,
          `🕳️ Trenches: ${summary.trenchCount}`,
          `🌰 Seedlings: ${summary.seedlingCount}`,
          `Total Markers: ${summary.markerCount}`,
          "",
          "--- Data ---",
          `GPS Points: ${summary.trailCount}`,
          `Reports: ${summary.reportCount}`,
          `Photos: ${summary.photoCount}`,
          "",
          "===================================",
          "  Generated by SpringHub",
          "  https://springhub.vercel.app",
          "===================================",
        ];

    const filename = isId
      ? `springhub-ringkasan-${Date.now()}.txt`
      : `springhub-summary-${Date.now()}.txt`;
