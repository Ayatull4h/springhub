...
>   - ⚠️ Jika vol123 gagal, coba `vol12345` (seed-test-accounts.ts mungkin belum dijalankan)
...
|**Harapan**|Ada cookie `guest_session_id` dengan random ID|
|**✅/❌**|**✅**|
|**Catatan**||
...
|**Harapan**|✅ Nama spring, lokasi tampil. ✅ Stats: pemantauan, restorasi, pohon, rorak, bibit, foto. ✅ Mini map. ✅ Timeline aktivitas. ✅ Galeri foto. **Jika muncul "spring tidak ditemukan"** → kemungkinan data spring belum ada di database. Submit laporan dengan `spring_name` terlebih dahulu|
|**✅/❌**|**✅**|
|**Catatan**||
...
|**Harapan**|Tidak bisa diubah. Nilai dikirim server-side dari `_captured_at`|
|**✅/❌**|**✅**|
|**Catatan**||
...
## Catatan Perbaikan (Update 19 Juni 2026)
...
### Fix Sesi 7 (19 Juni 2026) — Bug UAT Part 2

|TC|Masalah|Fix|Status|
|-|-|-|-|
|02, 12|Field province muncul di paling bawah|`activeForm` merge logic di report form: ganti `[...dbForm.fields, ...missingStaticFields]` (append) → merge berdasarkan urutan static schema pakai `staticForm.fields.map(sf => dbFieldMap.get(sf.id) \|\| sf)`|✅|
|34, 36, 85|Mini Map tidak muncul / Mapbox placeholder token gagal|Ganti Mapbox static map (token placeholder) → OSM `staticmap.openstreetmap.de` langsung + error fallback UI|✅|
|41|Offline province dropdown kosong / tidak muncul|Buat `lib/provinces.ts` (shared 38 provinsi). Tambah `case "province"` di `simple-offline-form.tsx`. Fix `offline-survey-map.tsx` pakai `INDONESIAN_PROVINCES`|✅|
|43, 44, 45, 46|Tombol simpan offline tidak ada respon di HP|Tambah `submitting` state (cegah double-click). `handleSubmit` pakai try/catch. `alert()` diganti inline error state. GPS tidak blocking submit. Tombol lebih besar (h-7) untuk touch target|✅|
|48|Report issue tidak bisa preview/hapus foto|Tambah thumbnail grid + × button per foto (pakai `URL.createObjectURL`)|✅|

### Detail Fix Sesi 7

1. **`lib/provinces.ts`** — File baru berisi daftar 38 provinsi Indonesia (shared constant, reusable semua form).
2. **`app/report/[slug]/page.tsx`** — Import `INDONESIAN_PROVINCES` dari lib, ganti inline `PROVINSI` array.
3. **`components/offline/simple-offline-form.tsx`** — Tambah `submitting` state + `submitError` state + try/catch handleSubmit + loader "Menyimpan..." + GPS tidak blocking submit. Tambah `case "province"` pakai `INDONESIAN_PROVINCES`. Tombol × foto diperbesar (h-7) untuk touch target HP.
4. **`components/offline/offline-survey-map.tsx`** — Import `INDONESIAN_PROVINCES`, pakai untuk `type === "province"` (sebelumnya kosong karena `field.options` undefined).
