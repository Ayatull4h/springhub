...
|2.2|Login admin|`admin@springhub.id` / `demo12345` — masuk|PASS|
...
|2.7|Tanpa huruf besar|`abcdefgh1` ditolak|PASS, tapi kalo saat sign in tulisan: password salah|
...
|3.9|Minimal foto|Coba approve foto < 1 per field — ditolak|PASS|
|3.10|Manajemen form|Daftar 5 form — klik lihat field|PASS|
|3.11|Edit form|Edit field → simpan — berubah|PASS|
|3.12|Soft delete form|Hapus → form nonaktif, data aman|PASS|
|3.13|Lihat form Survei|32 field dari A1\_tanggal s/d E3\_aksi|PASS|
|3.14|Setting map|Tipe titik peta + warna kategori|PASS|
|3.15|Ubah warna marker|Color picker — warna marker berubah|PASS|
...
|4.3|Auto WA|A3\_wa terisi dari profil|FAIL|
...
|4.8|3 foto|B2\_foto\_1, B3\_foto\_2, B4\_foto\_3 — min **1 foto per field** (total 3)|PASS, tapi terdapat tulisan: "Minimal 3 foto" dibawah field|
|4.9|Kirim|Isi required + 3 foto → sukses|PASS, tapi kalo submit foto totalnya lebih dari 5 akan cuman 5 foto yang muncul di review queue, foto lainnya akan terhilang|
...
|5.2|Deskripsi|"SATU FORM = SATU POHON"|FAIL, ada deskripsi|
...
|6.2|Field tersalin|Nama, sumber, lokasi tanam dari entri sebelumnya|FAIL, tidak muncul|
|6.3|GPS refresh|Posisi GPS baru setelah lanjut|FAIL, perlu geotag lagi|
...
|7.2|Activity marker (7)|Lingkaran abu 8px untuk tree-planting, rorak, dll — **tanpa** lingkaran 5km|FAIL, tree-planting denga warna hijau|
...
|7.5|Sub filter Sehat|Spring sehat aja|PASS, sub filter sehat ada 2, satu menunjukkan semua mata air, satu menunjukkan spring sehat aja|
|7.6|Sub filter Kritis|Spring kritis aja|PASS|
|7.7|Klik marker spring|Popup: nama, status, skor, link ke `/springs/\[id]`|PASS, map pada spring marker menunjakkan png, serta sangat burem dan pinnya kurang jelas di daerah mana|
|7.8|Klik marker tree-planting|Popup: jumlah + user (tanpa link detail jika gak punya springId)|PASS, kalo dipencet tidak ditemukan|
...
|9.2|Pilih nominal|Rp 20K, 50K, dll — field terisi|PASS, menurut ku sampingnya diberi nominal bukan hanya kata kata|
|9.3|Custom nominal|Min Rp 1.000, max Rp 100.000.000|PASS|
|9.4|Validasi tier|Pilih 20K tapi isi 1jt → ditolak|-, tidak bisa isi 1jt|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|11.1|Landing page|Semua section berubah|PASS|
|11.2|Admin panel|Sidebar, tabel, tombol gelap|PASS|
|11.3|Admin map|Background gelap, teks terbaca|PASS|
|11.4|Form|Form 32 field terbaca|PASS|
|11.5|Logo|Putih di dark, hitam di light|FAIL, di profil logo bibitku dan permintaanku tetap lightmode,serta email di donasi tetap putih|
...
|12.4|Auto-fill WA|Buka form → A3\_wa terisi dari profil|FAIL|
|12.5|Riwayat poin|Daftar poin pernah didapat|PASS|
|12.6|Logout|Kembali ke halaman utama|FAIL, tidak teredirect dan ada tulisan unauthorized.<br /><br />Tapi kalo pencet tombol keluar di profile, Kembali ke halaman Utama tetapi di atas kanan nama username tetap muncul|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|13.1|Tambah course|`/admin/courses` → tambah → simpan|FAIL, Invalid CSRF token|
|13.2|Edit form field|`/admin/forms` → edit → simpan|PASS, kalo ngedit berhasil, tapi kalo create Invalid CSRF token|
|13.3|Hapus form|Soft delete — jadi nonaktif|PASS|
|13.4|Ganti slug|Ditolak (punya laporan)|PASS|
|13.5|Tambah content|`/admin/content` → simpan → muncul di landing|FAIL, saat pencet simpan tidak ada yang terjadi|
...
|#|Yang Dicek|Cara Cek|Hasil (semua di laptop, karena di HP tidak ada form)|
|-|-|-|-|
|15.1|Halaman offline|`/offline` — tombol "Mulai Survey Offline"|PASS|
|15.2|Pilih form|Pilih Survei Mata Air 32 field|FAIL, masih pada form lama|
|15.3|Cache form|Form muncul di daftar offline|FAIL, saat coba di HP tidak berhasil tapi kalo di laptop berhasil tapi formnya yang lama|
|15.4|Isi offline (32 field)|Termasuk GPS auto-fill|PASS|
|15.5|Upload foto offline|3 foto (1 per field)|FAIL, masih pada form lama di Laptop|
|15.6|Submit offline|Simpan → sukses|PASS|
|15.7|Sinkronisasi|Online → `/offline` → Sinkronkan → terkirim|PASS, tapi ada tulisan gagal sinkron di Laptop|
...
|16.4|3 foto wajib|Upload 3 foto lokasi|FAIL, saat upload foto hanya ada nama foto bukan gambar fotonya |
|16.5|Komitmen|3 checkbox: lapor, review, publik — wajib centang|PASS|
|16.6|Kirim|Submit → sukses|FAIL, walaupun sudah upload foto tidak ada |
...
|17.2|Tab Survei|Laporan survei mata air muncul|PASS|
...
|17.6|Foto pagination|12 foto/klik — "Muat lebih banyak (12/68 foto)"|PASS|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|18.1|Marketplace|`/seedlings` — grid card, filter provinsi, search|PASS|
|18.2|Pagination|9 card/halaman + prev/next|-, kurang banyak card untuk ada lebih dari 1 halaman|
|18.3|Minta bibit|Klik "Minta" → isi jumlah → kirim (login)|PASS|
|18.4|WA aman|API tidak bocorkan nomor HP|PASS|
|18.5|Admin approve|Laporan seedling → approve → card aktif|FAIL, card tidak muncul di pasar bibit|
|18.6|Selesai 2 langkah|Penyedia klik "Selesai" + penerima klik "Selesai" → stok berkurang|-, tidak bisa membuat card baru untuk cek|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|19.1|Form publik|Buka `/report/kolaborasi-kemitraan` — tanpa login|FAIL, formulir tidak ditemukan|
...
|20.2|Ringan (60-79)|Agak Keruh, Berkurang, ancaman sedikit|FAIL, awalnya tertulis tercemar tapi saat refresh menjadi sehat|
|20.3|Berat (30-59)|Keruh, Naik Turun, banyak ancaman|FAIL, awalnya tertulis tercemar tapi saat refresh menjadi sehat|
|20.4|Kritis (<30)|Kering Total|FAIL, awalnya tertulis tercemar tapi saat refresh menjadi sehat|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|25.1|Backup jam 3 pagi|Cek `/root/backups/` — file .sql.gz|-, saat coba di Ubuntu perlu password|
...
|26.2|Nama dari session|A2\_nama\_surveyor terisi username|FAIL|
|26.3|WA dari profil|A3\_wa terisi nomor WA|FAIL|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|27.1|Label field berubah|Ganti bahasa ke English → buka form survei → label jadi "Spring Type", "Water Color", dll|PASS, tapi ada beberapa translation yang kurang akurat|
|27.2|Opsi select berubah|Buka form → opsi dropdown berubah Inggris (Clear, Murky, dll)|PASS|
|27.3|Kembali ke Indonesia|Ganti ke ID → label kembali ke Bahasa Indonesia|PASS|
