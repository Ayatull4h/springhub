...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|5.1|Form terbuka|`/report/tree-planting` — 16 field|PASS, akurasi GPS kalo valuenya taro lebih dari 15 masih bisa submit|
...
|6.4|Foto direset|Field foto kosong — wajib upload 3 baru|APSS|
...
|9.5|Invoice|**TERTUNDA** — butuh XENDIT\_SECRET\_KEY|PASS|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|11.1|Landing page|Semua section berubah|FAIL, logo pasar bibit masih tidak dark mode|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|13.1|Tambah course|`/admin/courses` → tambah → simpan|PASS, Dark mode tidak benar pas bikin kursus baru|
...
|15.5|Upload foto offline|3 foto (1 per field)|FAIL, 3 foto per field minimum|
|15.6|Submit offline|Simpan → sukses|PASS|
|15.7|Sinkronisasi|Online → `/offline` → Sinkronkan → terkirim|FAIL, sinkron gagal saat ku coba|
|15.8|PWA icon|Add to Home Screen — logo baru|PASS, tapi pada logo barunya terdapat kotak kotak putih abu abu|
|15.9|Offline langsung|Buka PWA offline → `/offline`|PASS|
|15.10|Auto-fill WA|Login → offline → WA terisi dari cache|FAIL, tidak ada data yang ter auto fill|
...
|16.6|Kirim|Submit → sukses|FAIL, walau sudah upload foto masih diminta minimal 3 foto|
...
|17.2|Tab Survei|Laporan survei mata air muncul|FAIL, laporan detail tentang mata air tidak muncul|
...
|18.5|Admin approve|Laporan seedling → approve → card aktif|PASS, card aktif tapi pas ku click cardnya detail dan deskripsi tidak sesuai dengan yang diisi di form, semuanya pohon Jati|
...
|26.4|Default C1\_warna|Default "Bening"|-|
...
|27.3|Kembali ke Indonesia|Ganti ke ID → label kembali ke Bahasa Indonesia|FAIL, di bagian survey spring, terdapat Bahasa Indoensia di pertanyaan Follow up willingness|
