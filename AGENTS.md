...
### 12 Agustus 2026 — Sesi 16: MODUL BELAJAR (Buku 12 Bab)
- **Output**: `MODUL-BELAJAR.md` (+ `MODUL-BELAJAR.pdf`, **461 halaman, 3.5MB**) — buku bahasa Indonesia menjelaskan SEMUA kode proyek (363 file, ±48.500 baris). Diunduh via GitHub, bukan tombol di aplikasi.
- **Struktur**: 12 bab — intro, arsitektur (diagram ASCII), peta kode (inventaris file), fondasi, kamar mesin (18 konstruk: if/else, ternary, async/await, try/catch, loop, map/filter, destructuring, optional chaining, regex, spread, Promise.all, setInterval, dll — semua dengan potongan kode asli + cerita "jika A maka B"), `lib/` (bab 5), API 94 route (bab 6), komponen 39 (bab 7), data/Prisma (bab 8), infrastruktur (bab 9), kerentanan + mitigasi (bab 10, 258 bagian 🛡️), glosarium (bab 11).
- **Format tiap topik**: `### <path>` → Potongan Kode Asli → Penjelasan Cerita → Konstruk → 🛡️ Kerentanan. Nilai sensitif diganti `xxx`.
