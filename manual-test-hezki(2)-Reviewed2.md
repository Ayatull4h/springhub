...
|1.6|Map muncul|Scroll ke peta — marker spring (warna) + aktivitas (abu) muncul|PASS, marker warna abu-abu tidak ada|
...
|3.14|Export spring|Buka `/api/admin/export?entity=spring` — CSV semua spring + health score|PASS|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|5.1|Form terbuka|`/report/tree-planting` — 16 field|PASS, tapi nomor WA tidak langsung terisi|
|5.2|Deskripsi|"SATU FORM = SATU POHON"|FAIL, tidak ada deskripsinya|
...
|7.2|Activity marker (7)|Lingkaran abu 8px untuk tree-planting, rorak, dll — **tanpa** lingkaran 5km|FAIL, activity marker warna hijau|
...
|7.5|Sub filter Sehat|Spring sehat aja|PASS|
...
|8.3|Leaderboard|Papan peringkat di landing page|FAIL, akun aku tidak masuk leaderboard|
...
|9.5|Invoice|**TERTUNDA** — butuh XENDIT\_SECRET\_KEY|XENDIT\_SECRET\_KEY is not set. Cannot create invoice.|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|11.1|Landing page|Semua section berubah|FAIL, di pasar bibit dark mode tidak berhasil semuanya|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|13.1|Tambah course|`/admin/courses` → tambah → simpan|PASS, kalo mau menghapus kursus tidak bisa, CSRF token invalid|
...
|13.5|Tambah content|`/admin/content` → simpan → muncul di landing|FAIL, tidak bisa simpan|
...
|14.3|Map types|`curl $API/api/map-points/types` → 4 tipe|{ASS|
...
|15.5|Upload foto offline|3 foto (1 per field)|FAIL, diminta 3 foto per field|
...
|15.10|Auto-fill WA|Login → offline → WA terisi dari cache|FAIL, tidak ada autofill|
...
|16.6|Kirim|Submit → sukses|FAIL, saat mau submit tulisan Wajib upload 3 foto lokasi proyek, walaupun sudah upload foto|
...
|17.2|Tab Survei|Laporan survei mata air muncul|FAIL, laporan tidak muncul|
...
|18.2|Pagination|9 card/halaman + prev/next|-, kurang banyak card untuk test|
|18.3|Minta bibit|Klik "Minta" → isi jumlah → kirim (login)|PASS|
|18.4|WA aman|API tidak bocorkan nomor HP|PASS|
|18.5|Admin approve|Laporan seedling → approve → card aktif|FAIL, tidak muncul di pasar bibit|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|19.1|Form publik|Buka `/report/kolaborasi-kemitraan` — tanpa login|FAIL, form tidak ditemukan|
...
|20.4|Kritis (<30)|Kering Total|FAIL, kering total saat cek dapet skore 55/100|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|25.1|Backup jam 3 pagi|Cek `/root/backups/` — file .sql.gz|-, kurang tau cara cek|
...
|26.4|Default C1\_warna|Default "Bening"|FAIL, tidak ada default|
|26.5|Default C8|Default "Observasi Sendiri"|FAIL, tidak ada default|
...
|#|Yang Dicek|Cara Cek|Hasil|
|-|-|-|-|
|27.1|Label field berubah|Ganti bahasa ke English → buka form survei → label jadi "Spring Type", "Water Color", dll|PASS, tapi kalo udah isi dan ganti bahasa, form yang sudah terisi hilang|
|27.2|Opsi select berubah|Buka form → opsi dropdown berubah Inggris (Clear, Murky, dll)|PASS, tapi kalo udah isi dan ganti bahasa, form yang sudah terisi hilang|
